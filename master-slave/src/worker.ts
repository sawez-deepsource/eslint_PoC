...
function sendMessage(msg: LintResult | WorkerError | MemorySample): Promise<void> {
...
async function runLint(task: LintTask): Promise<void> {
  ...
  try {
    ...
    // Send results back to master (await to ensure delivery before exit)
    await sendMessage({
      type: "result",
      workerId,
      results,
  ...
  // Small delay to ensure IPC buffer is flushed
